...
class MimeMailParser
{
     ...
     * @var array
     */
    private $attachment_streams = array();
    ...
    public function hasHeader($name, $part_id)
    {
        if (isset($this->parts[$part_id])) {
            $headers = $this->getPartHeaders($this->parts[$part_id]);
    ...
    /**
     * Return ids of all parsed parts
     *
     * @return array
     */
    ...
    public function findPartidsByContentType($content_type)
    {
        $ids = array();
        foreach ($this->parts as $id => $part) {
            if ($this->getPartContentType($part) == $content_type) {
                $ids[] = $id;
            }
        }
    ...
    public function getPartBody($part_id)
    {
        $part = $this->parts[$part_id];
        $headers = $this->getPartHeaders($part);
        $body = $this->decode($this->_getPartBody($part), array_key_exists('content-transfer-encoding', $headers) ? $headers['content-transfer-encoding'] : '');
        return $body;
    }
